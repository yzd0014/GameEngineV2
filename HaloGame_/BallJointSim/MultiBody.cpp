...
{	
	...
	tickCountSimulated++;
	if (m_MatlabSave) SaveDataToMatlab(20);
	if (m_HoudiniSave)
	{
		frameNum = 250;
...
bool eae6320::MultiBody::ClampRotationVector(_Vector& io_q, _Vector& io_qdot, int i)
{
	int jointPosIndex = 0;
	int jointVelIndex = 0;
	if (jointType[i] == FREE_JOINT_EXPO)
	{
		jointPosIndex = posStartIndex[i] + 3;
		jointVelIndex = velStartIndex[i] + 3;
	}
	else if (jointType[i] == BALL_JOINT_3D)
	{
		jointPosIndex = posStartIndex[i];
		jointVelIndex = velStartIndex[i];
	}
...
void eae6320::MultiBody::Integrate_q(_Vector& o_q, std::vector<_Quat>& o_quat, _Vector& i_q, std::vector<_Quat>& i_quat, _Vector& i_qdot, _Scalar h)
{
	for (int i = 0; i < numOfLinks; i++)
	{
		...
		{
			Math::QuatIntegrateV2(o_quat[i], i_quat[i], i_qdot.segment(velStartIndex[i], 3), h);
		}
	...
	std::vector<int>& i_jointType, std::vector<int>& i_posStartIndex)
{
	...
	for (int i = 0; i < numOfLinks; i++)
	{
		...
		else if (i_jointType[i] == BALL_JOINT)
		{
			//compute H
			o_H[i].resize(6, 3);
		...
		else if (i_jointType[i] == FREE_JOINT_EXPO)
		{
			...
			_Vector3 r = i_q.segment(i_posStartIndex[i] + 3, 3);
			_Scalar theta = r.norm();
...
void eae6320::MultiBody::ComputeMr(_Matrix& o_M, std::vector<_Matrix>& i_Ht)
{
...
{	
	...
	for (int i = 0; i < numOfLinks; i++)
	{
		...
		else if (jointType[i] == BALL_JOINT)
		{
			...
			_Vector3 gamma_theta;
			gamma_theta.setZero();
			gamma_theta = Math::ToSkewSymmetricMatrix(w_abs_world[i]) * R_global[i] * r_dot;
		...
		else if (jointType[i] == FREE_JOINT_EXPO)
		{
			...
			_Vector3 r_dot = i_qdot.segment(velStartIndex[i] + 3, 3);
			_Scalar theta = r.norm();
			_Scalar b = Compute_b(theta);
			_Scalar a = Compute_a(theta);
			_Scalar c = Compute_c(theta, a);
			...
			gamma[i].resize(6);
			gamma[i].setZero();
			gamma[i].block<3, 1>(3, 0) = Jdot_rdot;
		}
		else if (jointType[i] == HINGE_JOINT)
		{
			_Vector3 gamma_theta;
...
void eae6320::MultiBody::UpdateBodyRotation(_Vector& i_q, std::vector<_Quat>& i_quat, std::vector<int>& i_jointType, std::vector<int>& i_posStartIndex)
{
	for (int i = 0; i < numOfLinks; i++)
	{
		int j = parentArr[i];
		//update orientation
		if (i_jointType[i] == BALL_JOINT_4D || i_jointType[i] == BALL_JOINT)
		{
		...
		{
			R_local[i] = i_quat[i].toRotationMatrix();
			R_global[i] = i_quat[i].toRotationMatrix();
		}
		else if (i_jointType[i] == FREE_JOINT_EXPO)
		{
			_Vector3 r = i_q.segment(i_posStartIndex[i] + 3, 3);
...
void eae6320::MultiBody::AddRigidBody(int parent, int i_jointType, _Vector3 jointPositionChild, _Vector3 jointPositionParent, Assets::cHandle<Mesh> i_mesh, Vector3d i_meshScale, _Matrix3& i_localInertiaTensor)
{
	...
	if (i_jointType == BALL_JOINT_4D || i_jointType == BALL_JOINT)
	{
		velDOF.push_back(3);
		posDOF.push_back(4);
		totalVelDOF += 3;
	...
	else if (i_jointType == FREE_JOINT_EXPO)
	{
		velDOF.push_back(6);
		posDOF.push_back(6);
		totalVelDOF += 6;
