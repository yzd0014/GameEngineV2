...
void eae6320::Cloth::GenerateClothInitialPos()
{
	...
	initial_x1.resize(3, verticeCount);
	initial_x2.resize(3, verticeCount);
	{
		...
		for (uint16_t i = 0; i < clothMesh->GetVerticesCount(); i++) {
			...
			initial_x1(0, i) = newPos.x;
			initial_x1(1, i) = newPos.y;
			initial_x1(2, i) = newPos.z;
		}
	}
	{
		...
		for (uint16_t i = 0; i < clothMesh->GetVerticesCount(); i++) {
			...
			initial_x2(0, i) = newPos.x;
			initial_x2(1, i) = newPos.y;
			initial_x2(2, i) = newPos.z;
		}
	}
}
void eae6320::Cloth::InitializeCloth()
{
	...
	for (uint16_t i = 0; i < clothMesh->GetVerticesCount(); i++) {
		if (mode == 0)
		{
			clothMesh->m_pVertexDataInRAM[i].x = static_cast<float>(initial_x1(0, i));
			clothMesh->m_pVertexDataInRAM[i].y = static_cast<float>(initial_x1(1, i));
			clothMesh->m_pVertexDataInRAM[i].z = static_cast<float>(initial_x1(2, i));
			x = initial_x1;
		}
		else 
		{
			clothMesh->m_pVertexDataInRAM[i].x = static_cast<float>(initial_x2(0, i));
			clothMesh->m_pVertexDataInRAM[i].y = static_cast<float>(initial_x2(1, i));
			clothMesh->m_pVertexDataInRAM[i].z = static_cast<float>(initial_x2(2, i));
			x = initial_x2;
		}
	}
...
void eae6320::Cloth::Tick(const float i_secondCountToIntegrate) {
	...
	if(mode == 1) SelfCollisionDetection();
